"""
@File : rec_mobilenet_v3.py
@Author : CodeCat
@Time : 2021/7/15 下午9:25
"""
...
class MobileNetV3(nn.Module):
    def __init__(self, num_classes=1000, type='large'):
        ...
        if self.type == 'large':
            self.large_bottleneck = nn.Sequential(
                ...
                SEInverteBottleneck(in_channel=24, mid_channel=72, out_channel=40, kernel_size=5, use_se=True, activate='relu', stride=(2, 1)),
                SEInverteBottleneck(in_channel=40, mid_channel=120, out_channel=40, kernel_size=5, use_se=True, activate='relu', stride=1),
                SEInverteBottleneck(in_channel=40, mid_channel=120, out_channel=40, kernel_size=5, use_se=True, activate='relu', stride=1),
                SEInverteBottleneck(in_channel=40, mid_channel=240, out_channel=80, kernel_size=3, use_se=False, activate='hswish', stride=(2, 1)),
                ...
                SEInverteBottleneck(in_channel=112, mid_channel=672, out_channel=160, kernel_size=5, use_se=True, activate='hswish', stride=(2, 1)),
    ...
    def forward(self, x):
        ...
        if self.type == 'large':
            ...
            x = self.large_last_stage[:3](x)
        else:
            x = self.small_bottleneck(x)
            x = self.small_last_stage[:3](x)
...
if __name__ == '__main__':
    model = MobileNetV3()
    model.load_state_dict(model_zoo.load_url(model_urls["mobilenet_v3_large"]), strict=False)
    input = torch.rand((1, 3, 32, 100))
    out = model(input)
    print(out.shape)